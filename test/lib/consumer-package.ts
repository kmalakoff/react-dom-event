...
export function installPackedPackage(source: string, destination: string, archiveDirectory: string): void {
  ...
  const entries = Array.isArray(output) ? output : Object.values(output as Record<string, unknown>);
  if (entries.length !== 1 || typeof (entries[0] as { filename?: unknown })?.filename !== 'string') throw new Error('Expected one packed consumer package.');
  const filename = (entries[0] as { filename: string }).filename;
